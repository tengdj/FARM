...
//                                              PixPair *pixpairs, uint *pp_size, int8_t *flags, uint8_t category_count)
// {
...
// 			a_edge_id = (a_edge_id + 1) % (num_vertices - 1);
// 			a_param = 0.0;
//         } 
...
//             if (i < num_segments - 1 && (status[i] == 0 || status[i] == 2))
//                 continue;
...
//     kernel_filter_intersection<<<grid_size, block_size>>>(gctx->d_candidate_pairs + gctx->index, gctx->d_idealoffset, gctx->d_info, gctx->d_status, batch_size, (PixPair *)gctx->d_BufferInput, gctx->d_bufferinput_size, gctx->d_flags, gctx->category_count);
//     cudaDeviceSynchronize();
