...
__global__ void kernel_init(PolygonPair *d_pairs, RasterInfo *d_info, uint size, double *distance, double *max_box_dist)
{
...
__global__ void first_cal_box_distance(PolygonPair *pairs, RasterInfo *layer_info, uint16_t *layer_offset, uint8_t *status, double *max_box_dist, uint *global_level, uint size, BoxDistRange *buffer, uint *buffer_size, bool *resultmap)
{
    const int pair_id = blockIdx.x * blockDim.x + threadIdx.x;
    if (pair_id < size)
    {
...
__global__ void cal_box_distance(BoxDistRange *candidate, PolygonPair *pairs, RasterInfo *layer_info, uint16_t *layer_offset, uint8_t *status, double *max_box_dist, uint *global_level, uint *size, BoxDistRange *buffer, uint *buffer_size, bool *resultmap)
{
...
__global__ void kernel_refine(Batch *batches, Point *vertices, uint *size, double *distance, bool *resultmap, double *max_box_dist)
{
    const int bufferId = blockIdx.x * blockDim.x + threadIdx.x;
    if (bufferId < *size)
    {
...
uint cuda_within_polygon(query_context *gctx)
{
    ...
    log("The number of polygon pairs = %u", size);
    ...
    bool *h_resultmap = new bool[size];
    ...
    uint *h_pixelpairsize = new uint[size];
    uint *d_pixelpairsize = nullptr;
    CUDA_SAFE_CALL(cudaMalloc((void **)&d_pixelpairsize, size * sizeof(uint)));
    CUDA_SAFE_CALL(cudaMemset(d_pixelpairsize, 0, size * sizeof(uint)));
    memset(h_pixelpairsize, 0, size * sizeof(uint));

    int *h_pixelpairidx = new int[size];
    ...
    CUDA_SAFE_CALL(cudaMalloc((void **)&d_BufferOutput, 10UL * 1024 * 1024 * 1024));
...
// #endif
    ...
    // kernel_sort<<<grid_size, block_size>>>((BoxDistRange *)d_BufferInput, size, d_pixelpairsize, d_pixelpairidx);
    // cudaDeviceSynchronize();
    // check_execution("kernel_sort");

    timer.stopTimer();
    printf("kernel_sort: %f ms\n", timer.getElapsedTime());

    BoxDistRange *d_sortedpairs = nullptr;
    CUDA_SAFE_CALL(cudaMalloc((void **)&d_sortedpairs, h_bufferinput_size * sizeof(BoxDistRange)));
    ...
    // CUDA_SAFE_CALL(cudaMemcpy(&h_bufferinput_size, d_bufferinput_size, sizeof(uint), cudaMemcpyDeviceToHost));
    // printf("BUFFERINPUT_SIZE = %d\n", h_bufferinput_size);
    // BoxDistRange *h_BufferInput = new BoxDistRange[h_bufferinput_size];
    // cudaMemcpy(h_BufferInput, d_BufferInput, h_bufferinput_size * sizeof(BoxDistRange), cudaMemcpyDeviceToHost);
    // for(int i = 0; i < h_bufferinput_size; i ++)
    //     printf("%d %lf\n",h_BufferInput[i].pairId, h_BufferInput[i].minDist);
    ...
    while(true){
        ...
        kernel_refine<<<grid_size, block_size>>>((Batch *)d_BufferInput, gctx->d_vertices, d_bufferinput_size, d_distance, d_resultmap, d_max_box_dist);
        cudaDeviceSynchronize();
    ...
    for (int i = 0; i < size; i++)
    {
        if (h_distance[i] <= WITHIN_DISTANCE || h_resultmap[i])
            found++;
        printf("%lf\n", h_distance[i]);
    }
