...
uint cuda_within(query_context *gctx)
{
    ...
    for(int i = 0; i < point_polygon_pairs_size; i += batch_size)
    {
        ...
        timer.startTimer();
        first_cal_box_distance<<<grid_size, block_size>>>(d_pairs, gctx->d_points, gctx->d_idealoffset, gctx->d_info, gctx->d_status, gctx->d_min_box_dist, gctx->d_max_box_dist, size, (BoxDistRange *)gctx->d_BufferOutput, gctx->d_bufferoutput_size);
        cudaDeviceSynchronize();
