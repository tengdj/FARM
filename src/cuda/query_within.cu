...
// 		box &s_mbr = info[source.info_start].mbr;
...
// __global__ void first_cal_box_distance(IdealPair *pairs, Point *points, IdealOffset *idealoffset, RasterInfo *info, uint8_t *status, double *min_box_dist, double *max_box_dist, uint size, BoxDistRange *buffer, uint *buffer_size)
// {
//     const int pair_id = blockIdx.x * blockDim.x + threadIdx.x;
//     if (pair_id < size)
//     {
...
//         timer.startTimer();
//         kernel_init<<<grid_size, block_size>>>(d_pairs + start, gctx->d_points, gctx->d_idealoffset, gctx->d_info, size, gctx->d_distance, gctx->d_min_box_dist, gctx->d_max_box_dist);
...
//         timer.startTimer();
//         first_cal_box_distance<<<grid_size, block_size>>>(d_pairs + start, gctx->d_points, gctx->d_idealoffset, gctx->d_info, gctx->d_status, gctx->d_min_box_dist, gctx->d_max_box_dist, size, (BoxDistRange *)gctx->d_BufferOutput, gctx->d_bufferoutput_size);
//         cudaDeviceSynchronize();
...
//         timer.startTimer();
//         kernel_unroll<<<grid_size, block_size>>>((BoxDistRange *)gctx->d_BufferInput, d_pairs + start, gctx->d_points, gctx->d_idealoffset, gctx->d_offset, gctx->d_edge_sequences, gctx->d_bufferinput_size, (Task *)gctx->d_BufferOutput, gctx->d_bufferoutput_size);
//         cudaDeviceSynchronize();
