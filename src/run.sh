...
# do
# $BUILD_PATH/intersect -s $EXP_PATH/$DATA3 -t $EXP_PATH/$DATA4 -r -h -g -m $val >> output.txt
# done
# $BUILD_PATH/intersect -s $EXP_PATH/$DATA3 -t $EXP_PATH/$DATA4 -r -h -g -m 0.9 >> output.txt

# make clean
# make within_polygon USE_GPU=1 -j
# $BUILD_PATH/within_polygon -s $EXP_PATH/$DATA4 -r -h -g -m 0.0 >> output.txt
# $BUILD_PATH/within_polygon -s $EXP_PATH/$DATA4 -r -h -g -b 5000 --NLow 131072 >> output.txt
# $BUILD_PATH/within_polygon -s $EXP_PATH/$DATA4 -r -h -g -b 100000 -u 2 >> output.txt


# for val in $(awk 'BEGIN{for(i=0.0;i<=1.0;i+=0.1) printf "%.1f ", i}')
# do
# $BUILD_PATH/within_polygon -s $EXP_PATH/$DATA4 -r -h -g -b 1000000 -m $val >> output.txt
# done
# for ((i=0; i<=16; i++))
# do
#     para=$((2**i))
#     $BUILD_PATH/within_polygon -s $EXP_PATH/$DATA4 -r -h -g -b 5000 --NLow $para >> output.txt
# done
# for ((i=1; i<=10; i++))
# do
#     para=$((2**i))
